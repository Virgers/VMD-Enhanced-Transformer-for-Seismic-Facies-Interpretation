...
# Visualization part
def visualize_vmd_results(trace_index=0):
    # Get a sample trace and its VMD result
    original_signal = f[trace_index]
    vmd_result = u_array[trace_index]
    
    # Calculate reconstructed signal
    reconstructed_signal = np.sum(vmd_result, axis=0)
    
    # Create a figure with 5 subfigures
    plt.figure(figsize=(20, 16))
    gs = GridSpec(5, 1, figure=plt.gcf(), hspace=0.4)
    
    # Plot original signal
    ax1 = plt.subplot(gs[0])
    ax1.plot(original_signal, 'k', linewidth=2)
    ...
    for i in range(3):
        ...
        ax.plot(vmd_result[i], linewidth=2, label=f'IMF {i+1}')
    ...
    ax5.plot(original_signal, 'k', linewidth=2, label='Original')
    ax5.plot(reconstructed_signal, 'r--', linewidth=2, label='Reconstructed')
...
# Visualize multiple traces if needed
for trace_idx in [0, 10, 100]:  # Visualize a few sample traces
    visualize_vmd_results(trace_idx)