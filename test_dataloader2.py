...
class TSF_custom(Dataset):
    ...
    def __read_data__(self):
        ...
        if not self.is_vmd :
            ...
            # if we want to test the crossline we need to swap the first two axes and if we want to test last several sections we need to count from last
            # data_raw = np.swapaxes(data_raw, 1, 0) 
            # label_raw = np.swapaxes(label_raw, 1, 0) 
        ...
        split_datasets = self.split_data_label(data_raw, label_raw, mask_raw, self.flag)

        # split_datasets = self.split_data_label(data_raw, label_raw)
    ...
    def split_data_label(self, data_raw, label_raw, mask_raw, is_vmd):
        ...
        val_test_size =  int(val_test_proportion * num_samples)
