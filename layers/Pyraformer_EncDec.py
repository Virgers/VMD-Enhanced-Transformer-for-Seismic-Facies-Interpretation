...
class EncoderLayer(nn.Module):
    ...
    def __init__(self, configs, d_model, d_inner, n_head, dropout=0.1, normalize_before=True):
        super(EncoderLayer, self).__init__()

        self.slf_attn = AttentionLayer(
            FullAttention(configs.is_attn_mask, configs.att_mask_none,  factor=0,
...
class Encoder(nn.Module):
    ...
    def __init__(self, configs, window_size, inner_size):
        ...
        self.layers = nn.ModuleList([
            EncoderLayer(configs, configs.d_model, configs.d_ff, configs.n_heads, dropout=configs.dropout,
        ...
        self.enc_embedding = DataEmbedding(
            configs.seq_len, configs.enc_in, configs.d_model, configs.embed, configs.freq, configs.dropout)
    ...
    def forward(self, x_enc, x_mark_enc):
        ...
        seq_enc = self.enc_embedding(x_enc, x_mark_enc, self.embedding_flag, self.vmd_flag)
