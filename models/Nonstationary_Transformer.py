...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        # Embedding
        self.enc_embedding = DataEmbedding(configs.seq_len, configs.enc_in, configs.d_model, configs.embed, configs.freq,
                                           configs.dropout)
        ...
        if self.task_name == 'classification':
            ...
            # self.projection_cls = nn.Linear(configs.d_model, configs.seq_len, bias=True)
            self.projection_cls = nn.Linear(configs.d_model, configs.num_class2, bias=True)

        if self.vmd_flag:

            self.tau_learner = Projector(enc_in=configs.enc_in*8, seq_len=configs.seq_len, hidden_dims=configs.p_hidden_dims,
                                        hidden_layers=configs.p_hidden_layers, output_dim=1)
            
            self.delta_learner = Projector(enc_in=configs.enc_in*8, seq_len=configs.seq_len,
